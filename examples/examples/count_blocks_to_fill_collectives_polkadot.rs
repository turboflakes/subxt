// subxt metadata --url wss://sys.turboflakes.io:443/collectives-polkadot -f bytes > collectives_polkadot_metadata.scale
// subxt codegen --file collectives_polkadot_metadata.scale | rustfmt --edition=2018 --emit=stdout > collectives_polkadot_metadata.rs
// cargo run --example count_blocks_to_fill_collectives_polkadot
...
#[subxt::subxt(runtime_metadata_path = "../artifacts/collectives_polkadot_metadata.scale")]
pub mod node {}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api =
        OnlineClient::<PolkadotConfig>::from_url("wss://sys.turboflakes.io:443/collectives-polkadot").await?;

    let start_block_number = find_start_block(api.clone(), 1866972).await?;
...
async fn find_start_block(
    ...
    loop {
        if let Some(block_hash) = api.rpc().block_hash(Some(block_number.into())).await? {
            ...
            {
                println!(
                    "NewDesiredCandidates event seen at block #{block_number} -> https://polkadot.js.org/apps/?rpc=wss://sys.turboflakes.io:443/collectives-polkadot#/explorer/query/{block_number}",
                );
                break;
            } else {
                block_number += 1;
