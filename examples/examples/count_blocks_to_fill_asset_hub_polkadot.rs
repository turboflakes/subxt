...
// subxt codegen --file asset_hub_polkadot_metadata.scale | rustfmt --edition=2018 --emit=stdout > asset_hub_polkadot_metadata.rs
// cargo run --example count_blocks_to_fill_asset_hub_polkadot
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let api =
        OnlineClient::<PolkadotConfig>::from_url("wss://sys.turboflakes.io:443/statemint").await?;

    let start_block_number = find_start_block(api.clone(), 4334471).await?;
